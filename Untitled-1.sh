...
log_message() {
    ...
    # Si quieres ver los mensajes en la consola Y en el log, descomenta la línea de abajo:
    # echo "$(date '+%Y-%m-%d %H:%M:%S') - $message"
}

# Función para ejecutar comandos SQL en el contenedor MySQL (usando ROOT_USER y MYSQL_PWD)
execute_mysql_root_command() {
    local sql_command="$1"
    log_message "  Ejecutando SQL (ROOT): $sql_command"
    docker exec -i "$MYSQL_CONTAINER_NAME" sh -c "export MYSQL_PWD='$MYSQL_ROOT_PASSWORD'; echo \"$sql_command\" | mysql -u \"$MYSQL_ROOT_USER\"" >> "$SCRIPT_LOG_FILE" 2>&1
    if [ $? -ne 0 ]; then
        log_message "  ERROR: Falló la ejecución del comando SQL (ROOT): $sql_command"
        return 1
    fi
    return 0
}

# Función para ejecutar comandos SQL en una base de datos específica (usando ROOT_USER y MYSQL_PWD)
execute_mysql_root_db_command() {
    local db_name="$1"
    local sql_command="$2"
    log_message "  Ejecutando SQL (ROOT) en DB '$db_name': $sql_command"
    docker exec -i "$MYSQL_CONTAINER_NAME" sh -c "export MYSQL_PWD='$MYSQL_ROOT_PASSWORD'; echo \"$sql_command\" | mysql -u \"$MYSQL_ROOT_USER\" \"$db_name\"" >> "$SCRIPT_LOG_FILE" 2>&1
    if [ $? -ne 0 ]; then
        log_message "  ERROR: Falló la ejecución del comando SQL (ROOT) en $db_name: $sql_command"
        return 1
    fi
    return 0
}

# Función para ejecutar comandos SQL en el contenedor MySQL (usando MYSQL_USER y MYSQL_PWD para LOAD DATA)
execute_mysql_user_command() {
    local sql_command="$1"
    log_message "  Ejecutando SQL (USER): $sql_command"
    docker exec -i "$MYSQL_CONTAINER_NAME" sh -c "export MYSQL_PWD='$MYSQL_PASSWORD'; echo \"$sql_command\" | mysql -u \"$MYSQL_USER\"" >> "$SCRIPT_LOG_FILE" 2>&1
    if [ $? -ne 0 ]; then
        log_message "  ERROR: Falló la ejecución del comando SQL (USER): $sql_command"
        return 1
    fi
    return 0
}

# Función para limpiar los directorios temporales locales
cleanup() {
    log_message "--- Iniciando limpieza de directorios locales ---"
    if [ -d "$LOCAL_DOWNLOAD_PATH" ]; then
        log_message "Eliminando directorio temporal de descarga: $LOCAL_DOWNLOAD_PATH"
        rm -rf "$LOCAL_DOWNLOAD_PATH"
    fi
    if [ -d "$TEMP_UNZIP_DIR" ]; then
        log_message "Eliminando directorio temporal de descompresión: $TEMP_UNZIP_DIR"
        rm -rf "$TEMP_UNZIP_DIR"
    fi
    log_message "--- Limpieza local completa ---"
}
...
# 1. Crear directorios temporales locales
...
# 6. Iterar sobre cada archivo ZIP descargado, extraer CSV e insertar en MySQL
find "$LOCAL_DOWNLOAD_PATH" -maxdepth 1 -name "*.zip" -print0 | while IFS= read -r -d $'\0' zip_file; do
    ...
    # 6.1. Validar si el archivo ZIP ya fue procesado
    ...
    # 6.2. Detectar el nombre del CSV dentro del ZIP
    ...
    # 6.3. Descomprimir el archivo CSV detectado en su subdirectorio temporal local
    ...
    # --- 6.4. CREAR BASE DE DATOS Y TABLA (Solo una vez por ejecución del script) ---
    if [ "$TABLE_CREATED" = false ]; then
        ...
        CREATE_TABLE_SQL="CREATE TABLE IF NOT EXISTS \`$TABLE_NAME\` ($SQL_COLUMNS);"
        ...
        if [ $? -ne 0 ]; then
            log_message "  ERROR: No se pudo crear la tabla '$TABLE_NAME' en la base de datos '$DB_NAME'. Verifique permisos de MYSQL_ROOT_USER o estructura de tabla."
            rm -rf "$current_unzip_path"
            continue
        fi
    ...
    # 6.5. Copiar el CSV extraído al contenedor MySQL
    ...
    # 6.6. Asegurar permisos de lectura para el archivo copiado dentro del contenedor
    ...
    # 6.7. Insertar datos en MySQL usando LOAD DATA INFILE (usando MYSQL_USER y MYSQL_PWD)
    log_message "  -> Insertando datos de '$CONTAINER_FULL_PATH' en '$DB_NAME.$TABLE_NAME' usando LOAD DATA INFILE..."
    LOAD_SQL="LOAD DATA INFILE '$CONTAINER_FULL_PATH' INTO TABLE \`$DB_NAME\`.\`$TABLE_NAME\` FIELDS TERMINATED BY '$CSV_DELIMITER' ENCLOSED BY '\"' LINES TERMINATED BY '\n' IGNORE 1 LINES;"
    ...
    # 6.8. Eliminar el archivo CSV del contenedor MySQL después de la inserción exitosa
    ...
    # 6.9. Registrar el archivo ZIP como procesado (¡solo si todo fue exitoso hasta aquí!)
...
log_message "Todos los archivos procesados han sido insertados en MySQL."
